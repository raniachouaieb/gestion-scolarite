...
function set_active_child_menu($item){
    \Modules\Core\Walkers\MenuWalker::setCurrentChildMenuItem($item);
}
...
function get_profile_url($file_id,$size="thumb",$resize = true){
    if(empty($file_id)) return false;
    return \App\Helpers\PdfHelper::urlAvatar($file_id);
}

function get_avatarStudent_url($file_id,$size="thumb",$resize = true){
    if(empty($file_id)) return false;
    return \App\Helpers\PdfHelper::urlAvatarStudent($file_id);
}

function get_filePDF_url($file_id){
    if(empty($file_id)) return false;
    return \App\Helpers\PdfHelper::url($file_id);
}

function get_bulletinfile_url($file_id){
    if(empty($file_id)) return false;
    return \App\Helpers\BulletinFileHelper::url($file_id);
}
...
if (! function_exists('assets')) {
    ...
    {
        return app('url')->asset($path, $secure) . HtmlHelper::getAutoVersion($path);
    }
}

function HasError($errors, $name){
    if($errors->has($name)){
        return '<span class="invalid-feedback" style="border-color: #fbe1e3;color: #e73d4a;float: left;padding-bottom: 10px; " role="alert"><strong>'.$errors->first($name).'</strong></span>';
    }
}
